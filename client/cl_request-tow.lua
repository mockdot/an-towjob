...
AddEventHandler('an-tow:receiveTowRequest', function(target, plate, coords)
    exports["npwd"]:createSystemNotification({
        ...
        onConfirm = function()
            ...
            local blipTowLoc = AddBlipForCoord(coords.x, coords.y, coords.z)
            SetBlipAsShortRange(blipTowLoc, false)
            SetBlipSprite(blipTowLoc, 68)
            SetBlipColour(blipTowLoc, 0)
            SetBlipScale(blipTowLoc, 0.7)
            SetBlipDisplay(blipTowLoc, 6)
            SetBlipRoute(blipTowLoc, true)
            Wait(120000)
            SetBlipRoute(blipTowLoc, false)
            RemoveBlip(blipTowLoc)
        end,
        onCancel = function()
